...
def fixture_data(html_file_path):
    ...
    table_dict = {item["Event"]: {"Quantity": item["Quantity"],
...
def extract_player_profile(html_file_path):
    ...
    player_price = int(soup.find('p', class_='player-value').get_text(strip=True).replace('מחיר ', '').replace('M', ''))

    player_position = data_utility.position_mapping.get(player_position, player_position)
