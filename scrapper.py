...
try:
    ...
    # Sleep for a few seconds to allow the table to load
    time.sleep(2)
    ...
    for row in rows[:4]:  # Skip the header row
        ...
        # Find the modal with the h3 text "פרופיל שחקן"
        ...
        player_dir = f'players/Player{player_index}'
        ...
        if table:
            ...
            for row in rows_stats:
                ...
                fixture_number = re.search(
                    r'\d+', fixture_text).group() if re.search(r'\d+', fixture_text) else None
    ...
    # To keep the browser window open after execution, comment out the following line
    driver.quit()
    print("HHHKH")
